...
#include "Power_Component_tracer.h"
...
static sc_string default_scope_name = "Power_Component";
...
{
	return Power_Component_meta_feature_names[featureId];
}
...
{
	return Power_Component_meta_state_names[stateId];
}

static yet_error dispatchMessage(yet_scope *scope, yet_message * msg, char *fqn);

void power_Component_init_sc_tracer(yet_sc_tracer *tracer, Power_Component* machine, sc_string name)
{
	if (tracer != sc_null) {
...
static yet_error dispatchMessage(yet_scope *scope, yet_message * msg, char *fqn)
{
	...
	if(strcmp(member, "on") == 0) {
		...
		power_Component_raise_on(tracer->machine);
		return 0;
	}
	if(strcmp(member, "off") == 0) {
		...
		/* TODO: this is not optimal check if we can get rid of it */
		tracer->skip_raised_in_event++; 
		power_Component_raise_off(tracer->machine);
		return 0;
	}
	if(strcmp(member, "time") == 0) {
		sc_integer converted = (sc_integer)atol(msg->value);
		power_Component_set_time(tracer->machine, converted);
		return 0;
	}
	if(strcmp(member, "kWh") == 0) {
		sc_integer converted = (sc_integer)atol(msg->value);
		power_Component_set_kWh(tracer->machine, converted);
		return 0;
	}
	if(strcmp(member, "kilowatt") == 0) {
		sc_integer converted = (sc_integer)atol(msg->value);
		power_Component_set_kilowatt(tracer->machine, converted);
		return 0;
	}
	if(strcmp(member, "hour") == 0) {
		sc_integer converted = (sc_integer)atol(msg->value);
		power_Component_set_hour(tracer->machine, converted);
		return 0;
	}
	if(strcmp(member, "totalPower") == 0) {
		sc_integer converted = (sc_integer)atol(msg->value);
		power_Component_set_totalPower(tracer->machine, converted);
		return 0;
	}
	if(strcmp(member, "totalHour") == 0) {
		sc_integer converted = (sc_integer)atol(msg->value);
		power_Component_set_totalHour(tracer->machine, converted);
		return 0;
	}
	if(strcmp(member, "device.isOn") == 0) {
		sc_boolean converted = (msg->value[0] == 't');
		power_Component_device_set_isOn(tracer->machine, converted);
		return 0;
	}
