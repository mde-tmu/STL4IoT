...
#include "SmartTrafficLightSystem_tracer.h"
...
static sc_string default_scope_name = "SmartTrafficLightSystem";

static yet_value_serializer feature_value_serializer[24];
...
static void init_feature_value_serializer() {
	...
	feature_value_serializer[17] = yet_serialize_string;
	feature_value_serializer[18] = yet_serialize_string;
...
{
	return SmartTrafficLightSystem_meta_feature_names[featureId];
}
...
{
	return SmartTrafficLightSystem_meta_state_names[stateId];
}

static yet_error dispatchMessage(yet_scope *scope, yet_message * msg, char *fqn);

void smartTrafficLightSystem_init_sc_tracer(yet_sc_tracer *tracer, SmartTrafficLightSystem* machine, sc_string name)
{
	if (tracer != sc_null) {
...
static yet_error dispatchMessage(yet_scope *scope, yet_message * msg, char *fqn)
{
	...
	if(strcmp(member, "system.on") == 0) {
		...
		smartTrafficLightSystem_system_raise_on(tracer->machine);
		return 0;
	}
	if(strcmp(member, "system.off") == 0) {
		...
		/* TODO: this is not optimal check if we can get rid of it */
		tracer->skip_raised_in_event++; 
		smartTrafficLightSystem_system_raise_off(tracer->machine);
		return 0;
	}
	if(strcmp(member, "message") == 0) {
		sc_string converted = msg->value;
		smartTrafficLightSystem_set_message(tracer->machine, converted);
		return 0;
	}
	if(strcmp(member, "WiFi_connection") == 0) {
		sc_boolean converted = (msg->value[0] == 't');
		smartTrafficLightSystem_set_wiFi_connection(tracer->machine, converted);
		return 0;
	}
	if(strcmp(member, "Temp_value") == 0) {
		sc_integer converted = (sc_integer)atol(msg->value);
		smartTrafficLightSystem_set_temp_value(tracer->machine, converted);
		return 0;
	}
	if(strcmp(member, "Power_input") == 0) {
		sc_integer converted = (sc_integer)atol(msg->value);
		smartTrafficLightSystem_set_power_input(tracer->machine, converted);
		return 0;
	}
	if(strcmp(member, "Power_total") == 0) {
		sc_integer converted = (sc_integer)atol(msg->value);
		smartTrafficLightSystem_set_power_total(tracer->machine, converted);
		return 0;
	}
	if(strcmp(member, "Power_required") == 0) {
		sc_integer converted = (sc_integer)atol(msg->value);
		smartTrafficLightSystem_set_power_required(tracer->machine, converted);
		return 0;
	}
	if(strcmp(member, "system.isOn") == 0) {
		sc_boolean converted = (msg->value[0] == 't');
		smartTrafficLightSystem_system_set_isOn(tracer->machine, converted);
		return 0;
	}
	if(strcmp(member, "system.light_status") == 0) {
		sc_string converted = msg->value;
		smartTrafficLightSystem_system_set_light_status(tracer->machine, converted);
		return 0;
	}
	if(strcmp(member, "system.crossing_status") == 0) {
		sc_string converted = msg->value;
		smartTrafficLightSystem_system_set_crossing_status(tracer->machine, converted);
		return 0;
	}
	if(strcmp(member, "system.timer") == 0) {
		sc_integer converted = (sc_integer)atol(msg->value);
		smartTrafficLightSystem_system_set_timer(tracer->machine, converted);
		return 0;
	}
	if(strcmp(member, "SensorInput.detection_time") == 0) {
		sc_integer converted = (sc_integer)atol(msg->value);
		smartTrafficLightSystem_SensorInput_set_detection_time(tracer->machine, converted);
		return 0;
	}
	if(strcmp(member, "SensorInput.timeout_time") == 0) {
		sc_integer converted = (sc_integer)atol(msg->value);
		smartTrafficLightSystem_SensorInput_set_timeout_time(tracer->machine, converted);
		return 0;
	}
	if(strcmp(member, "SensorOutput.counter") == 0) {
		sc_integer converted = (sc_integer)atol(msg->value);
		smartTrafficLightSystem_SensorOutput_set_counter(tracer->machine, converted);
		return 0;
	}
	if(strcmp(member, "SensorOutput.activity") == 0) {
		sc_boolean converted = (msg->value[0] == 't');
		smartTrafficLightSystem_SensorOutput_set_activity(tracer->machine, converted);
		return 0;
	}
