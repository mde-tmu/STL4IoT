...
public class Statecharts_Initializer extends DashboardFrame_Editor{
	...
	public void readSmartLightSystemDATA(SmartHubSystem SmartHubSystem) {
		...
		boolean led1_status = SmartHubSystem.smartLightsHUB().getLed1_isON();
		boolean led2_status = SmartHubSystem.smartLightsHUB().getLed2_isON();
		
		//LED BRIGHTNESS
		int led1_lum = (int)SmartHubSystem.smartLightsHUB().getLed1_lum();
		int led2_lum = (int)SmartHubSystem.smartLightsHUB().getLed2_lum();
		...
		if(Light_status) {
			if(Light_usage) {
				...
				smartLight_switch1.setEnabled(true);
				smartLight_switch2.setEnabled(true);
			}
			else if(!Light_usage) {
				usage_status = "Master Switch: OFF";
				smartLight_switch1.setEnabled(false);
				smartLight_switch2.setEnabled(false);
			}
			...
			//LED 1 STATUS
			if(led1_status) {
		        smartLight_switch1.setText("LED 1 Status: ON ("+led1_lum+")");
		        smartLight_led1.setEnabled(true);
		        smartLight_led1.setValue(led1_lum);
			}
			else if(!led1_status) {
		        smartLight_switch1.setText("LED 1 Status: OFF");
		        smartLight_led1.setEnabled(false);
			}
			
			//LED 2 STATUS
			if(led2_status) {
		        smartLight_switch2.setText("LED 2 Status: ON ("+led2_lum+")");
		        smartLight_led2.setEnabled(true);
		        smartLight_led2.setValue(led2_lum);
			}
			else if(!led2_status) {
		        smartLight_switch2.setText("LED 2 Status: OFF");
		        smartLight_led2.setEnabled(false);
			}
	...
	public void ButtonClicksHandler(SmartHubSystem SmartHubSystem) {
      ...
      //LED LIGHT 1 UNIT SWITCH
        smartLight_switch1.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
				anti_freeze = new Thread(new Runnable() {
					...
					public void run() {
						playSound("Audio/click.wav");
						boolean bulbisOn = SmartHubSystem.smartLightsHUB().getLed1_isON();
						
						if(bulbisOn) {
							SmartHubSystem.getSL_HUB().system1().raiseToggle();
						}
						else {
							SmartHubSystem.getSL_HUB().system1().raiseToggle();
						}
       ...
       //LED LIGHT 2 UNIT SWITCH
        smartLight_switch2.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
				anti_freeze = new Thread(new Runnable() {
					...
					public void run() {
						playSound("Audio/click.wav");
						boolean bulbisOn = SmartHubSystem.smartLightsHUB().getLed2_isON();
						
						if(bulbisOn) {
							SmartHubSystem.getSL_HUB().system2().raiseToggle();
						}
						else {
							SmartHubSystem.getSL_HUB().system2().raiseToggle();
						}
        ...
        smartLight_led1.addChangeListener((ChangeListener) new ChangeListener() {

        	 @Override
             public void stateChanged(ChangeEvent e) {
                 // TODO Auto-generated method stub
        		 SmartHubSystem.getSL_HUB().system1().setInput_brightness(smartLight_led1.getValue());
             }
        });
        smartLight_led2.addChangeListener((ChangeListener) new ChangeListener() {

       	 @Override
            public void stateChanged(ChangeEvent e) {
                // TODO Auto-generated method stub
       		 SmartHubSystem.getSL_HUB().system2().setInput_brightness(smartLight_led2.getValue());
            }
       });
